  ...
  return (
    <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 mb-6">
      <div className="flex items-center gap-2.5 mb-2 text-rose-400 font-semibold text-sm">
        ...
        <span>System Bottleneck Detected ({bottlenecks.length} components affected)</span>
      </div>
        ...
        {bottlenecks.map((item) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-rose-300 font-medium">{item.nodeId}</span>
              <span className="text-[10px] text-slate-400">({item.nodeType})</span>
            </div>

            <div className="flex items-center gap-4 text-[11px] font-mono">
              <span className="text-slate-400">Latency: <span className="text-rose-400 font-bold">{item.latencyMs}ms</span></span>
              <span className="text-slate-400">Error Rate: <span className="text-amber-400 font-bold">{(item.errorRate * 100).toFixed(1)}%</span></span>
            </div>
          </div>
        ))}
      </div>
